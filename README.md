...
```sh
git clone git@github.com:NIBB-Neurophysiology-Lab/SlowFast.git
cd SlowFast
```

`DA_dataset`をSlowFastのディレクトリ直下に配置し、データセットの作成を行います。
...
モデルは既に学習済みとします。結果は`gradcam_out`フォルダに保存されます。


```sh
docker compose exec slowfast /bin/bash
python3 tools/run_net.py --cfg configs/DA_dataset/da_dataset_gradcam.yaml
```
