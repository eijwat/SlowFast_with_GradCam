...
class TestMeter:
    ...
    def finalize_metrics(self, ks=(1, 2)):
        ...
        if self.multi_label:
            ...
            # labelsとpredsをcsvに書き出す
            with open("results.csv", "w") as f:
                f.write("label,pred\n")
                for label, pred in zip(self.video_labels, self.video_preds):
                    f.write(str(label.item()) + "," + str(pred.argmax().item()) + '\n')
...
class TrainMeter:
    ...
    def log_iter_stats(self, cur_epoch, cur_iter):
        ...
        if not self._cfg.DATA.MULTI_LABEL:
            ...
            stats["top2_err"] = self.mb_top5_err.get_win_median()
        if self.multi_loss is not None:
...
class ValMeter:
    ...
    def log_epoch_stats(self, cur_epoch):
        ...
        if self._cfg.DATA.MULTI_LABEL:
            ...
            stats["min_top2_err"] = self.min_top5_err
