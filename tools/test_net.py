...
from slowfast.datasets.utils import tensor_denormalize

from slowfast.gradcam_3d import LayerGradHookContext, gradcam_3d, plot_movie
...
def perform_test(test_loader, model, test_meter, cfg, writer=None):
    ...
    for cur_iter, (inputs, labels, video_idx, time, meta) in enumerate(
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if not cfg.GRADCAM.ENABLE:
                preds = model(inputs)
            else:
                with LayerGradHookContext(model, eval("model." + cfg.GRADCAM.LAYER)) as hook:
                    with torch.enable_grad():
                        # Perform the forward pass.
                        preds = model(inputs)
                        input_shape = inputs[0].shape
                        input_cpu = tensor_denormalize(
                            inputs[0].cpu().permute(0, 2, 3, 4, 1),
                            cfg.DATA.MEAN,
                            cfg.DATA.STD,
                        ).permute(0, 4, 1, 2, 3)
                        target_class = 1
                        ...
                        cam = gradcam_3d(feature, gradient, input_shape)
                        plot_movie(
                            input_cpu[0],
                            cam[0],
                            f"{cur_iter}",
...
def test(cfg):
    ...
    for view, test_meter in zip(cfg.TEST.NUM_TEMPORAL_CLIPS, test_meters):
        ...
        result_string = (
            "_p{:.2f}_f{:.2f}_{}a{} Top2 Acc: {} MEM: {:.2f} f: {:.4f}"
            "".format(
                params / 1e6,
                flops,
                view,
